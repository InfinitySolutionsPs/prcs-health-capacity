...
export async function GET(){
  try{
    ...
    const db=getRawDb();
    const [hospitals,administrations,departments,jobTitles,cadreTypes,projects,projectJobs,staffing]=await db.batch([
      ...
      db.prepare("SELECT pj.id,pj.project_id AS projectId,p.name AS projectName,pj.job_title_id AS jobTitleId,j.name AS jobTitle,j.job_code AS jobCode,pj.salary,pj.coverage FROM project_jobs pj JOIN projects p ON p.id=pj.project_id JOIN job_titles j ON j.id=pj.job_title_id ORDER BY p.name,j.name"),
    ...
    return NextResponse.json({hospitals:hospitals.results,administrations:administrations.results,departments:departments.results,jobTitles:jobTitles.results,cadreTypes:cadreTypes.results,projects:projects.results,projectJobs:projectJobs.results,staffing:staffing.results});
