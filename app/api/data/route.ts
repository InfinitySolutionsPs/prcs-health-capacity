...
export async function GET(){
  try{
    ...
    const db=getRawDb();
    const [hospitals,administrations,departments,jobTitles,cadreTypes,staffing]=await db.batch([
      ...
      db.prepare("SELECT s.id, h.id AS hospitalId, h.name AS hospitalName, d.id AS departmentId, d.administration_id AS administrationId, COALESCE(a.name,d.division) AS division, d.name AS departmentName, s.job_title_id AS jobTitleId, COALESCE(j.name,s.job_title) AS jobTitle, s.required, CASE WHEN EXISTS(SELECT 1 FROM employees LIMIT 1) THEN (SELECT COUNT(*) FROM employees e WHERE e.status='على رأس عمله' AND e.facility=h.name AND COALESCE(e.administration,'')=COALESCE(a.name,d.division,'') AND COALESCE(e.department,'')=d.name AND e.job_title=COALESCE(j.name,s.job_title)) ELSE s.available END AS available, MAX(s.required-(CASE WHEN EXISTS(SELECT 1 FROM employees LIMIT 1) THEN (SELECT COUNT(*) FROM employees e WHERE e.status='على رأس عمله' AND e.facility=h.name AND COALESCE(e.administration,'')=COALESCE(a.name,d.division,'') AND COALESCE(e.department,'')=d.name AND e.job_title=COALESCE(j.name,s.job_title)) ELSE s.available END),0) AS gap FROM staffing s JOIN departments d ON d.id=s.department_id JOIN hospitals h ON h.id=d.hospital_id LEFT JOIN administrations a ON a.id=d.administration_id LEFT JOIN job_titles j ON j.id=s.job_title_id ORDER BY h.name,division,d.name,jobTitle")
    ]);
    return NextResponse.json({hospitals:hospitals.results,administrations:administrations.results,departments:departments.results,jobTitles:jobTitles.results,cadreTypes:cadreTypes.results,staffing:staffing.results});
