...
async function ensureAuthSchema(){const db=getRawDb();await db.prepare("CREATE TABLE IF NOT EXISTS system_users (id INTEGER PRIMARY KEY AUTOINCREMENT,auth_user_id TEXT,username TEXT,email TEXT NOT NULL,name TEXT NOT NULL,role TEXT NOT NULL DEFAULT 'viewer',active INTEGER NOT NULL DEFAULT 1,password_hash TEXT,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)").run();await db.prepare("CREATE TABLE IF NOT EXISTS system_metadata (key TEXT PRIMARY KEY,value TEXT NOT NULL)").run();const cols=(await db.prepare("PRAGMA table_info(system_users)").all()).results as {name:string}[];const names=new Set(cols.map(c=>c.name));if(!names.has("username"))await db.prepare("ALTER TABLE system_users ADD COLUMN username TEXT").run();if(!names.has("password_hash"))await db.prepare("ALTER TABLE system_users ADD COLUMN password_hash TEXT").run();return db}
