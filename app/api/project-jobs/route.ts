...
export async function GET() {
  ...
  const rows = await getRawDb().prepare("SELECT pj.id,pj.project_id AS projectId,p.name AS projectName,pj.job_title_id AS jobTitleId,j.name AS jobTitle,j.job_code AS jobCode,pj.salary,pj.coverage FROM project_jobs pj JOIN projects p ON p.id=pj.project_id JOIN job_titles j ON j.id=pj.job_title_id ORDER BY p.name,j.name").all();
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const salary = String(body.salary ?? "").trim(), coverage = String(body.coverage ?? "").trim();
    if (!Number.isInteger(projectId) || !Number.isInteger(jobTitleId) || !salary || !coverage) return NextResponse.json({ error: "اختر المشروع والمسمى وأدخل الراتب ونسبة التغطية" }, { status: 400 });
    const db = getRawDb();
    ...
    const result = await db.prepare("INSERT INTO project_jobs(project_id,job_title_id,salary,coverage) VALUES(?,?,?,?)").bind(projectId, jobTitleId, salary, coverage).run();
